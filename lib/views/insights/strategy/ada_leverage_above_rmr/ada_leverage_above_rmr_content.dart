...
final adaLeverageAboveRmrProvider = FutureProvider<List<LeverageData>>((
  ref,
) async {
  ...
  for (final iAsset in indigoAssets) {
    ...
    final rmr = iAsset.rmr;
    final mcr = iAsset.maintenanceRatio;
...
class AdaLeverageAboveRmrContent extends HookConsumerWidget {
  const AdaLeverageAboveRmrContent({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return ref
        .watch(adaLeverageAboveRmrProvider)
        .when(
          data: (leveragesData) => SingleChildScrollView(
            child: Column(
              children: [
                Container(
